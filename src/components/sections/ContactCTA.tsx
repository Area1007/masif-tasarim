    ...
    <section id="iletisim" className="border-b border-paper/10 bg-ink-soft py-24 text-paper sm:py-32 lg:py-40">
      <Container className="grid gap-16 lg:grid-cols-12 lg:gap-12">
        <div className="lg:col-span-5">
          <Reveal>
            <Eyebrow>İletişim & Teklif</Eyebrow>
            ...
            <p className="mt-8 max-w-md text-[16px] leading-[1.8] text-paper/65">
              Aklınızdaki mekânı kısaca anlatın; ihtiyaçlarınızı değerlendirip size özel bir tasarım ve uygulama
              teklifiyle dönüş yapalım.
            </p>
          </Reveal>
          ...
          <Reveal delay={120} className="mt-12 space-y-6">
            <div>
              ...
              <div className="mt-2 space-y-1">
                {siteConfig.phones.map((p) => (
                  <a key={p.href} href={p.href} className="block font-display text-3xl transition-colors hover:text-oak">
                    {p.label}
                  </a>
                ))}
              </div>
            ...
            {siteConfig.email && (
              <div>
                ...
                <a href={`mailto:${siteConfig.email}`} className="mt-2 block text-lg hover:text-oak">
                  {siteConfig.email}
                </a>
              </div>
            )}
            <a
              href={whatsappLink("Merhaba, projem hakkında bilgi almak istiyorum.")}
              target="_blank"
